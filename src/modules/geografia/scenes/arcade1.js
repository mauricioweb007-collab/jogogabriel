/* =====================================================================
   ...
   • Jangada Radical — corrida automática com pulo e pulo duplo
     (estilo carrinho de mina de Donkey Kong Country / Sonic);
...
(function () {
  ...
  PQ.register({ id: 'w1_jangada', world: 1, boss: 'c1s5', ref: 'Donkey Kong Country / Sonic', title: 'Jangada Radical', icon: 'veleiro', c1: '#1f8fd0', c2: '#0b3a5c', music: 'corrida', medals: [300, 700, 1200], unit: 'pts',
    ...
    how: 'A jangada anda sozinha. Aperte **Espaço** (ou toque) para **pular**; no ar, aperte de novo para o **pulo duplo**. Cuidado com as **gaivotas** lá em cima! Você tem **3 vidas**.' }, function (api) {
    const sc = { cam: { x: 0, y: 0 }, t: 0 };
    const SEA = 172, J = { x: 86, y: SEA, vy: 0, air: false, jumps: 0, inv: 0, star: 0 };
    let dist = 0, speed = 130, playing = false, spawnT = 1.2, itemT = 0.6, time = 0, obs = [], items = [], dead = false;
    ...
    function spawn() {
      ...
      const k = r < 0.34 ? 'pedra' : r < 0.58 ? 'tronco' : r < 0.78 + lvl * 0.05 ? 'ave' : 'onda';
      const o = { k, x: E.W + 30 };
    ...
    sc.update = function (dt) {
      ...
      if (!playing) return;
      time += dt; speed = Math.min(270, 130 + time * 1.2); dist += speed * dt; api.extra('cronometro', Math.max(0, Math.ceil(150 - time)) + 's');
      ...
      spawnT -= dt; if (spawnT <= 0) { spawn(); spawnT = Math.max(0.75, 1.6 - time * 0.006) + Math.random() * 0.5; }
      ...
      obs.forEach((o) => { o.x -= speed * dt * (o.k === 'ave' ? 1.25 : 1); });
      items.forEach((it) => { it.x -= speed * dt; });
      const box = { x: J.x - 16, y: J.y - 34, w: 30, h: 32 };
    ...
    sc.draw = function (g) {
      ...
      if (!(J.inv > 0 && Math.floor(sc.t * 16) % 2)) {
        ...
        if (J.star > 0 && x) x.glow(c, 0, -14, 34, 'hsl(' + (sc.t * 400 % 360) + ',90%,60%)', 0.7);
        c.fillStyle = '#6b3f1d'; for (let i = 0; i < 4; i++) c.fillRect(-20, -5 + i * 0 - (i % 2), 40, 5 - (i % 2)); c.fillStyle = '#8a5a2e'; c.fillRect(-20, -6, 40, 3);
        c.fillStyle = '#5a3a1a'; c.fillRect(-2, -44, 2, 40);
        c.fillStyle = '#fff8e6'; c.beginPath(); c.moveTo(0, -44); c.lineTo(20, -12); c.lineTo(0, -10); c.closePath(); c.fill(); c.fillStyle = '#e5484d'; c.fillRect(2, -30, 10, 3);
        c.drawImage(C.gabrielSide(GEO.eco.look(), J.air ? 'jump' : 'idle', sc.t), -16, -30);
        c.restore();
    ...
    sc.dbg = { end() { api.lives = 1; J.inv = 0; J.star = 0; hurt(); } };
    return sc;
  });
  ...
  PQ.register({ id: 'w1_colunas', world: 1, boss: 'c1s5', ref: 'Columns (Mega Drive) / Puyo Puyo', title: 'Colunas do Mosaico', icon: 'diamante', c1: '#8a3fd0', c2: '#2e1250', music: 'labirinto', medals: [800, 2000, 4000], unit: 'pts',
    ...
    how: '**← →** movem a coluna, **↑ / Espaço** trocam a ordem das cores, **↓** desce rápido. Junte **3 iguais** (em pé, deitado ou na diagonal). A **estrela** limpa todas as peças da cor onde cair! **3 vidas**.' }, function (api) {
    const sc = { cam: { x: 0, y: 0 }, t: 0 };
    ...
    sc.update = function (dt) {
      ...
      const iv = Math.max(0.18, 0.85 - (lvl - 1) * 0.08);
