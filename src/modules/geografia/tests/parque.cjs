...
(async () => {
  ...
  const lock0 = await p.$$eval('.pq-card', (els) => els.map((e) => e.classList.contains('lock')));
  ok(lock0.length === 4 && !lock0[0] && lock0.slice(1).every(Boolean), 'no começo só a Memória está liberada');
  await p.evaluate(() => { GG.ui.closeAll(); ['c1s1', 'c1s2', 'c1s3', 'c1s4', 'c1s5'].forEach((id) => { GEO.save.S.stages[id] = { done: true, plays: 1, playsDay: {}, best: { score: 1, medal: 'ouro' } }; }); GEO.parque.open(); });
  await p.waitForTimeout(300);
  const lock1 = await p.$$eval('.pq-card', (els) => els.map((e) => e.classList.contains('lock')));
  ok(lock1.every((l) => !l), 'com 5 fases concluídas os 4 minijogos liberam');
  ...
  await p.click('.pq-card[data-game="memoria"]'); await p.waitForTimeout(500); await clearDialogs(p); await p.waitForTimeout(400);
  ...
  await p.click('.pq-card[data-game="arara"]'); await p.waitForTimeout(500); await clearDialogs(p); await p.waitForTimeout(300);
  ...
  await p.click('.pq-card[data-game="cesta"]'); await p.waitForTimeout(500); await clearDialogs(p); await p.waitForTimeout(300);
  ...
  await p.click('.pq-card[data-game="quebra"]'); await p.waitForTimeout(500); await clearDialogs(p); await p.waitForTimeout(600);
  ...
  ok(after.coins === before.coins && after.xp === before.xp, 'minijogos não mexem em EcoMoedas nem XP');
  ...
  ok(saved === 4, 'recordes gravados no save de Geografia (4 jogos)');
